...
search_terms: dict[str, list[tuple[str, str]]] = {
    "base2": [("scyther", "10")],
...
for set_id, terms in search_terms.items():
    with open(f"data/metadata/cards/{set_id}.json", encoding="utf-8") as f:
        cards: list[dict[str, str]] = json.load(f)
    for name_term, number in terms:
        for c in cards:
            if name_term in c["name"].lower() and c.get("number") == number:
                print(
                    f"{set_id:>6}  {c['id']:>20}  {c['name']}  #{c.get('number', '')}"
                )
                break
...
for set_file in ["neo1", "neo2", "neo3", "neo4"]:
    try:
        with open(f"data/metadata/cards/{set_file}.json", encoding="utf-8") as f:
            cards = json.load(f)
        for c in cards:
            if "jumpluff" in c["name"].lower():
                print(
                    f"{set_file:>6}  {c['id']:>20}  {c['name']}  #{c.get('number', '')}"
                )
